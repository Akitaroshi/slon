...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const prevButton = document.querySelector('.features-prev');
    const nextButton = document.querySelector('.features-next');
    let currentSlide = 0;
...
function openModal(modalId) {
    ...
    document.querySelector(`#${modalId} .modal`).classList.add('active');
}
...
function login() {
    ...
    if (username === 'admin' && password === 'admin') {
        ...
        // Здесь должна быть проверка обычных пользователей
...
function register() {
    ...
    // Здесь должно быть сохранение пользователя
...
function showAdminPanel() {
    ...
    adminControls.innerHTML = `
        <button class="action-button" onclick="openModal('addSellerModal')">
            <i class="ph ph-plus"></i>
            <span>Добавить продавца</span>
        </button>
    `;

    document.querySelector('.filters').appendChild(adminControls);
}

function addSeller() {
    const name = document.getElementById('sellerName').value;
    const avatar = document.getElementById('sellerAvatar').value;
    const rating = document.getElementById('sellerRating').value;
    const deals = document.getElementById('sellerDeals').value;
    const minAmount = document.getElementById('sellerMinAmount').value;
    const maxAmount = document.getElementById('sellerMaxAmount').value;

    // Здесь должно быть добавление продавца в базу данных
    closeModal('addSellerModal');
    alert('Продавец успешно добавлен');
}

// Проверяем админа при загрузке страницы
